...
Initialize AI logic SDK with the `firebase-mcp-server` tool:

```bash
mcp_firebase-mcp-server_firebase_init(
  features: {
    ailogic: { app_id: "YOUR_FIREBASE_APP_ID" }
  }
)
```
...
Use the `genkit-mcp-server` to lookup deeper AI implementation guides:
- **Usage Guide**: `mcp_genkit-mcp-server_get_usage_guide(language: "js")`
- **General Docs**: `mcp_genkit-mcp-server_lookup_genkit_docs`
